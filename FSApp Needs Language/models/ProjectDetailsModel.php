...
class ProjectDetailsModel {
    ...
    public function createPayment($projectId, $payerUsername, $paymentName, $amount, $beneficiaries) {
        global $db;
        // Esto es a modo de debug, se puede quitar
        // Verificar si el usuario existe
        $stmt = $db->prepare('SELECT COUNT(*) FROM users WHERE username = ?');
        $stmt->execute([$payerUsername]);
        $userExists = $stmt->fetchColumn();

        if (!$userExists) {
            throw new Exception("El usuario '$payerUsername' no existe en la base de datos.");
        }
    ...
    public function editPayment($paymentId, $paymentName, $amount, $beneficiaries, $payer) {
        global $db;
